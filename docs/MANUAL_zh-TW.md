...
*上圖：這是服務表單的樣子。在畫面最上方，你會看到一排按鈕——左邊有「儲存」按鈕，右邊有「開啟服務」按鈕。往下看，表單分成幾個區塊：最上方是「服務名稱」欄位（這是必填的），接下來是「服務網址」欄位。在表單左側中間位置，你會看到 Logo 上傳區（一個方形的圖片上傳框）。表單右側有「卡片顏色」「分類」「內部負責人」等欄位。最下方則是討論區。以下步驟 3-8 都是在這個表單上操作。*
...
> 點擊「開啟服務」時，系統會自動把網址補全為 `https://slack.com`。
...
> **沒有 Logo 怎麼辦？**
>
> 如果你沒有上傳 Logo，系統會自動使用服務名稱的**首字母**作為替代顯示。例如「Slack」會顯示一個大大的「S」。所以即使沒有圖片，卡片上也不會空白。

---

#### 第六步：選擇卡片顏色

1. 找到 **卡片顏色** 欄位（參見上方表單截圖，在表單右側，是一個下拉選單）
...
| 服務 | 建議網址 | 建議分類 |
|------|----------|----------|
| GitHub | `github.com` | 開發 |
| Notion | `notion.so` | 文件 |
| Figma | `figma.com` | 設計 |
| Google Workspace | `workspace.google.com` | 生產力 |
| AWS Console | `aws.amazon.com` | 雲端與基礎架構 |
| Jira | `atlassian.net` | 專案管理 |

> **小提示：關於 Logo**
>
> 建議為每個服務上傳對應的品牌 Logo 圖片，這樣在看板上更容易辨識。如果沒有上傳 Logo，系統會自動顯示服務名稱的首字母。
...
*上圖：外部夥伴登入 Portal 後看到的畫面。在畫面中央，你會看到服務卡片牆——只會顯示被分享給他的服務，其他公司內部的服務他完全看不到。每張卡片上有服務 Logo（或首字母）、服務名稱，以及右下角的「開啟」按鈕。如果只分享了 2 個服務，這裡就只會出現 2 張卡片。*
...
*上圖：看板視圖——在畫面中央，每張卡片代表一個服務。仔細看卡片的組成：左側有一條顏色標示（代表分類顏色），中間顯示服務 Logo（或首字母）和名稱，下方有彩色的分類標籤，右下角是「開啟服務」按鈕。點擊這個按鈕可以直接打開服務網站。*

**卡片上有什麼？**

| 元素 | 說明 |
|------|------|
| Logo / 首字母 | 服務的視覺識別（有 Logo 顯示 Logo，否則顯示名稱首字母） |
...
*上圖：Portal 服務清單頁面。在畫面中央，你會看到服務卡片排列成網格狀。每張卡片的構成是：上方有服務 Logo（或首字母），中間顯示服務名稱（如「Slack」「Figma」），下方有一個明顯的「開啟」按鈕。如果服務數量較少，可能只有 1-3 張卡片；如果較多則會自動排列成多列。*
...
**症狀**：點擊「開啟服務」後，網址跟預期不同

**原因**：可能是手動輸入了 `http://`

**說明**：
- 如果你輸入 `slack.com`，系統在開啟服務時會自動補成 `https://slack.com`
...
| **首字母回退** | 當服務沒有上傳 Logo 時，系統自動顯示名稱首字母 | 「Slack」顯示「S」、「GitHub」顯示「G」 |
...
這裡提供上傳服務 Logo 的最佳實務建議。

> **Logo 與首字母回退機制**
>
> 服務中心採用 2 層視覺回退機制：
> 1. **Logo** — 如果有上傳 Logo 圖片，優先顯示 Logo
> 2. **首字母** — 如果沒有上傳 Logo，系統自動取服務名稱的第一個字元作為替代顯示
...
| 服務 | Logo 取得方式 |
|------|---------------|
| Slack | 到 slack.com/media-kit 下載官方 Logo |
| GitHub | 到 github.com/logos 下載官方 Logo |
| Figma | 到 figma.com/using-the-figma-brand 下載官方 Logo |
| Google | 到 about.google/brand-resource-center 下載官方 Logo |
| AWS | 到 aws.amazon.com/architecture/icons 下載官方 Logo |

> **小提示**
>
> 大多數知名服務都有提供官方 Logo 下載頁面（通常在「Brand」或「Press」頁面）。搜尋「服務名稱 + logo download」或「服務名稱 + brand assets」即可找到。
